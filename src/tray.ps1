...
$script:APP_VERSION = "v14.0.0-rc.1"
...
$script:PATCH_HISTORY = @(
    ...
    # ────────────────────────────────────────────────────────────────────────
    @{ Version = "v14.0.0-rc.1"; Date = "2026-05-07"; Notes = @(
        @{ Tag = "NEW";      Text = 'Stage 4 (.NET 8 migration): server.exe is now an ASP.NET Core minimal-API binary (net8.0) replacing the legacy Node.js + @yao-pkg/pkg build. All 26 routes ported with byte-equivalent HTTP behaviour modulo documented intentional differences (Content-Length vs chunked, no Keep-Alive timeout header, etc.). Process tree and memory profile differ from v12.0.1 -- testers may see lower steady-state memory.' }
        @{ Tag = "NEW";      Text = 'Stage 4.10 (Discord RPC): new C# DiscordRpcService + DiscordRpcThrottle (2000ms throttle, latest-wins coalescer) replaces the JS discord_rpc.js. Built on Lachee.DiscordRPC 1.2.1.24 NuGet (only new runtime dependency in V14). Visible to Discord users only -- behaviour mirrors v12.0.1 (paused/playing states, large/small images, Listen button).' }
        @{ Tag = "NEW";      Text = 'Stage 4.9 (album-art cascade): full 11-source cascade rewritten in C# (SmtcSource, SoundCloudDirect, OsuScraper, WebhookArt, SoundCloudOembed, Deezer, iTunes, MusicBrainz, SmtcFallback, YouTube, BingImage). enrichByTitle Deezer/iTunes search runs before lock; art and duration resolve concurrently via Task.WhenAll. May find art for tracks the legacy server could not.' }
        @{ Tag = "NEW";      Text = 'Stage 4.11 (launcher port-clearing): MastersFM.exe now clears port 4242 of any stale listener before spawning server.exe. Resolves the launcher-vs-server race where Kestrel exited cleanly within ~1s if a prior server.exe was still bound. Same logic tray.ps1 already had for non-skipServerLaunch case.' }
        @{ Tag = "NEW";      Text = 'Stage 5.1 (.NET 8 migration): tray_native.dll now built via dotnet build targeting netstandard2.0 (loadable by Windows PowerShell 5.1). Replaces csc.exe + .NET Framework 4.x build. tray.ps1 loads it via Add-Type -Path identically; all 9 public types resolve in PS5.1. csc.exe rollback path preserved as $UseDotnetTrayNative=$false.' }
        @{ Tag = "IMPROVED"; Text = 'Build pipeline: _full_rebuild.ps1 default flags now $true for launcher, audio_spectrum, customize, server, and tray_native (.NET 8). install_bootstrapper remains on csc.exe ($UseDotnet8Bootstrapper=$false) pending a real CA cert -- Bitdefender flags the self-signed bootstrapper pattern. RC1 ships hybrid: .NET 8 for the runtime stack, csc.exe rollback paths preserved as emergency revert.' }
        @{ Tag = "IMPROVED"; Text = 'tray.ps1 unchanged in this release except for this version string and patch-note entry. The tray host (MastersFM_Tray.exe / tray_launcher.cs) is also unchanged. Stage 6 (tray_launcher dissolution) is folded into Stage 7 cutover per the V14 plan; no work happens in this release.' }
        @{ Tag = "NEW";      Text = 'Release framing: this is v14.0.0-rc.1 (release candidate). Cumulative ship of Stages 1-3 + Stage 4 (sub-stages 4.1-4.11) + Stage 5 MINIMAL (sub-stages 5.1, 5.4, 5.5). Soak data is thin (6h overnight + 49min listening + a fresh 6h pre-ship soak). Please report anything unexpected in the Discord #v14-rc-feedback channel; a stable v14.0.0 follows after RC1 feedback.' }
    ) },
    @{ Version = "v12.3.0"; Date = "2026-05-04"; Notes = @(
        @{ Tag = "NEW";      Text = 'Stage 3 (.NET 8 migration): customize.exe (WebView2 host) now built via dotnet publish targeting net8.0-windows with PublishReadyToRun=true. Replaces the old csc.exe build path. No user-visible behaviour change: WebView2 window, frameless rounded corners, localhost:4242/customize navigation all preserved. WebView2 NuGet 1.0.2420.47 used (same version previously shipped in MSI).' }
        @{ Tag = "NEW";      Text = 'Stage 3 (.NET 8 migration): install_bootstrapper.cs ported to net8.0-windows with fix for Assembly.GetEntryAssembly().Location .NET 8 behaviour (now uses Environment.ProcessPath). Bootstrapper build remains disabled ($UseDotnet8Bootstrapper=$false) pending a real CA cert -- Bitdefender flags the self-signed pattern. The .csproj is ready to activate.' }
        @{ Tag = "IMPROVED"; Text = 'Build pipeline: _full_rebuild.ps1 gains $UseDotnet8Customize flag (currently $true) and $UseDotnet8Bootstrapper flag (currently $false, disabled). Set $UseDotnet8Customize=$false to roll back to the legacy csc.exe customize build without other changes.' }
        @{ Tag = "IMPROVED"; Text = 'Companion artifacts customize.dll, customize.runtimeconfig.json, and customize.deps.json now bundled in MSI alongside customize.exe for the .NET 8 apphost pattern (same as MastersFM.dll and audio_spectrum.dll added in Stages 1-2).' }
        @{ Tag = "IMPROVED"; Text = 'Directory.Build.props mechanism verified with 4 .NET 8 projects: all InternalName checks pass (MastersFM.dll, audio_spectrum.dll, customize.dll, install_bootstrapper.dll), confirming no apphost cross-contamination as more projects were added.' }
    ) },
    @{ Version = "v12.2.0"; Date = "2026-05-04"; Notes = @(
        @{ Tag = "NEW";      Text = 'Stage 2 (.NET 8 migration): audio_spectrum.exe now built via dotnet publish targeting net8.0-windows with PublishReadyToRun=true. Replaces the old csc.exe + netstandard.dll facade build path. No user-visible behaviour change: all four audio backends (WASAPI loopback, MME WaveIn, WDM-KS exclusive, ASIO) preserved. STA thread requirement for ASIO preserved ([STAThread] on Main). HTTP/SSE port 4243 interface and {f, b} frame shape unchanged.' }
        @{ Tag = "IMPROVED"; Text = 'NAudio 2.2.1 now referenced as individual NuGet packages (NAudio.Core, NAudio.Wasapi, NAudio.WinMM, NAudio.Asio) via audio_spectrum.csproj, replacing the manual /reference: flags and netstandard.dll facade needed for the .NET Framework 4.7.2+ csc.exe build. Output DLL set is identical to the pre-Stage-2 MSI.' }
        @{ Tag = "IMPROVED"; Text = 'Build pipeline: _full_rebuild.ps1 gains $UseDotnet8AudioSpectrum flag (currently $true). Set to $false to roll back to the legacy csc.exe build via build_tools\ps2exe\_build_spectrum.ps1 without other changes.' }
        @{ Tag = "NEW";      Text = 'Companion artifacts audio_spectrum.dll, audio_spectrum.runtimeconfig.json, and audio_spectrum.deps.json now bundled in MSI alongside audio_spectrum.exe (required for .NET 8 framework-dependent deployment).' }
    ) },
    @{ Version = "v12.1.0"; Date = "2026-05-04"; Notes = @(
        @{ Tag = "IMPROVED"; Text = 'Stage 0 — build tooling: switched server.exe bundler from vercel/pkg (archived) to @yao-pkg/pkg v6.19.0 (community drop-in fork). No user-visible change; eliminates a build-time dependency on unmaintained tooling.' }
        @{ Tag = "NEW";      Text = 'Stage 1 (.NET 8 migration) — MastersFM.exe (the launcher) is now compiled from net8.0-windows via dotnet publish with PublishReadyToRun=true, targeting x64. Startup performance improved (~30-50% faster cold-start via R2R pre-JIT). All behavior identical: Job Object, AUMID, Task Manager app-grouping, single-instance mutex, server.exe/audio_spectrum.exe/MastersFM_Tray.exe lifecycle management unchanged.' }
        @{ Tag = "IMPROVED"; Text = 'Build pipeline: _full_rebuild.ps1 gains $UseDotnet8Launcher flag (currently $true). Set to $false to roll back to the legacy csc.exe .NET Framework 4.x launcher instantly without other changes.' }
        @{ Tag = "NEW";      Text = '.NET 8 migration plan (V14_NET8_MIGRATION_PLAN.md) drafted via 5-agent research swarm + Opus system-architect. Stages 2-8 are defined and ready to execute; user reviews plan before proceeding.' }
    ) },
